...
export interface CarouselItem {
  ...
  imageSrc?: string; // optional image for background
}
...
function CarouselItem({ item, index, itemWidth, round, trackItemOffset, x, transition }: CarouselItemProps) {
  ...
  return (
    <motion.div
      ...
      style={{
        width: itemWidth,
        height: round ? itemWidth : '500px',
        rotateY: rotateY,
        ...(round && { borderRadius: '50%' }),
        backgroundImage: item.imageSrc ? `url(${item.imageSrc})` : undefined,
        backgroundSize: item.imageSrc ? 'cover' : undefined,
        backgroundPosition: item.imageSrc ? 'center' : undefined,
      }}
